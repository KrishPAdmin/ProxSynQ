...
ssh -tt "${PI_USER}@${PI_HOST}" "PI_USER='${PI_USER}' VM1='${VM1}' VM2='${VM2}' VM3='${VM3}' bash -s" <<'REMOTE_PI'
set -euo pipefail

mkdir -p /home/krishadmin/proxsyncq-rpi/control_ui/templates

cat > /home/krishadmin/proxsyncq-rpi/control_ui/templates/index.html <<'HTML'
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  ...
  <style>
    ...
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .grid-2 {
      display: grid;
      grid-template-columns: minmax(0, 1.55fr) minmax(36rem, 0.9fr);
      gap: 1.5rem;
      align-items: start;
    }
...
<body>
  ...
  <!-- ui-version-direct-fix-v3 -->
</body>
</html>
HTML

echo "== rebuild control_ui =="
sudo docker compose -f /home/krishadmin/proxsyncq-rpi/docker-compose.yml build --no-cache control_ui
sudo docker compose -f /home/krishadmin/proxsyncq-rpi/docker-compose.yml up -d control_ui
sleep 5
sudo docker compose -f /home/krishadmin/proxsyncq-rpi/docker-compose.yml ps
echo
curl -fsS http://127.0.0.1:8080 | grep -i "ui-version-direct-fix-v3"
echo

repair_exporter() {
  local host="$1"
  echo "== node_exporter on ${host} =="
  ssh -o BatchMode=yes -o StrictHostKeyChecking=no -tt "${PI_USER}@${host}" 'sudo bash -s' <<'REMOTE_VM'
set -euo pipefail
export DEBIAN_FRONTEND=noninteractive
apt-get update -y
apt-get install -y prometheus-node-exporter curl ca-certificates procps iproute2 >/dev/null 2>&1 || true
...
curl -fsS --max-time 5 http://127.0.0.1:9100/metrics | sed -n '1,2p' || true
REMOTE_VM
  echo
}

detect_agent_service_from_vm2() {
  ssh -o BatchMode=yes -o StrictHostKeyChecking=no "${PI_USER}@${VM2}" 'bash -s' <<'REMOTE_VM'
set -euo pipefail
pid="$(sudo ss -ltnp "( sport = :8000 )" 2>/dev/null | sed -n 's/.*pid=\([0-9]\+\).*/\1/p' | head -n1)"
if [ -n "$pid" ]; then
  while read -r unit _; do
    mainpid="$(sudo systemctl show -p MainPID --value "$unit" 2>/dev/null || true)"
    if [ "$mainpid" = "$pid" ]; then
      echo "$unit"
      exit 0
    fi
  done < <(sudo systemctl list-units --type=service --all --no-legend 2>/dev/null)
fi
sudo systemctl list-unit-files --type=service --no-legend 2>/dev/null | awk '{print $1}' | grep -Ei 'proxsyncq|agent|fastapi|uvicorn|api' | head -n1 || true
REMOTE_VM
}
...
repair_agent() {
  local host="$1"
  local ref_service="$2"
  echo "== node agent on ${host} =="
  ssh -o BatchMode=yes -o StrictHostKeyChecking=no -tt "${PI_USER}@${host}" "REF_SERVICE='${ref_service}' sudo bash -s" <<'REMOTE_VM'
set -euo pipefail
echo "host=$(hostname)"
if curl -fsS --max-time 5 http://127.0.0.1:8000/health >/dev/null 2>&1; then
  echo "health already ok"
  curl -fsS --max-time 5 http://127.0.0.1:8000/health || true
  exit 0
fi

candidates=""
if [ -n "${REF_SERVICE:-}" ]; then
  candidates="${REF_SERVICE}"
fi

extra="$(systemctl list-unit-files --type=service --no-legend 2>/dev/null | awk '{print $1}' | grep -Ei 'proxsyncq|agent|fastapi|uvicorn|api' || true)"
for svc in $extra; do
  case " $candidates " in
    *" $svc "*) ;;
...
for svc in $candidates; do
  ...
  sleep 3
  if curl -fsS --max-time 5 http://127.0.0.1:8000/health >/dev/null 2>&1; then
    echo "recovered_with_service=$svc"
    break
  fi
done
...
if ! curl -fsS --max-time 5 http://127.0.0.1:8000/health >/dev/null 2>&1; then
  while read -r name; do
    ...
    sleep 3
    if curl -fsS --max-time 5 http://127.0.0.1:8000/health >/dev/null 2>&1; then
      echo "recovered_with_container=$name"
      break
    fi
  done < <(docker ps -a --format '{{.Names}} {{.Ports}}' 2>/dev/null | awk '/8000->|:8000->|0.0.0.0:8000|:::8000/{print $1}')
fi

ss -ltnp | grep ':8000' || true
curl -fsS --max-time 5 http://127.0.0.1:8000/health || true
echo
systemctl list-unit-files --type=service --no-legend 2>/dev/null | awk '{print $1}' | grep -Ei 'proxsyncq|agent|fastapi|uvicorn|api' || true
REMOTE_VM
  echo
}
...
direct_checks() {
  local host="$1"
  echo "== direct checks from Pi to ${host} =="
  printf "/health: "
...
for host in "${VM1}" "${VM2}" "${VM3}"; do
  repair_exporter "$host"
done

REF_SERVICE="$(detect_agent_service_from_vm2 || true)"
echo "Detected reference agent service: ${REF_SERVICE:-none}"
echo

for host in "${VM1}" "${VM2}" "${VM3}"; do
  repair_agent "$host" "${REF_SERVICE:-}"
done

sudo docker compose -f /home/krishadmin/proxsyncq-rpi/docker-compose.yml restart prometheus
sleep 12
...
echo "== UI API check =="
curl -fsS http://127.0.0.1:8080/api/dashboard | sed -n '1,60p'
echo
REMOTE_PI
