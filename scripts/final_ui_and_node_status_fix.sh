  ...
  <style>
    ...
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .page-shell {
      ...
      max-width: 100%;
      margin: 0 auto;
      padding: calc(var(--headerH) + 1.4rem) 1rem 4rem 1rem;
    }
    ...
    .heading-sec__main {
      ...
      font-size: clamp(2.8rem, 2.9vw, 4.3rem);
      text-transform: uppercase;
      letter-spacing: 3px;
      position: relative;
      font-weight: 900;
    ...
    .grid-2 {
      display: grid;
      grid-template-columns: minmax(0, 1.55fr) minmax(34rem, 0.85fr);
      gap: 1.5rem;
      align-items: start;
    }
...
echo
curl -fsS http://127.0.0.1:8080 | grep -i "Cluster Operations Dashboard"
REMOTE

echo
echo "== repairing node services from Pi =="
...
apt-get update -y
apt-get install -y prometheus-node-exporter curl ca-certificates lsof procps
...
if [ -n "$pid" ]; then
  unit="$(sudo systemctl status "$pid" --no-pager 2>/dev/null | awk '/\.service/ {for(i=1;i<=NF;i++) if($i ~ /\.service$/){gsub(/^[^A-Za-z0-9._-]+/, "", $i); print $i; exit}}')"
  if [ -n "$unit" ]; then
    echo "$unit"
    exit 0
  fi
fi
systemctl list-unit-files --type=service --no-legend 2>/dev/null | awk '{print $1}' | grep -Ei 'proxsyncq|node.*agent|sync.*agent|job.*api|uvicorn|fastapi' | head -n1 || true
REMOTE_VM
}
...
repair_agent() {
  ...
  ssh -o BatchMode=yes -o StrictHostKeyChecking=no -tt "${PI_USER}@${host}" "AGENT_SERVICE=$service sudo bash -s" <<'REMOTE_VM'
set -euo pipefail
echo "host=$(hostname)"
...
candidate=""
if [ -n "${AGENT_SERVICE:-}" ] && systemctl list-unit-files --type=service --no-legend 2>/dev/null | awk '{print $1}' | grep -qx "$AGENT_SERVICE"; then
  candidate="$AGENT_SERVICE"
else
  candidate="$(systemctl list-unit-files --type=service --no-legend 2>/dev/null | awk '{print $1}' | grep -Ei 'proxsyncq|node.*agent|sync.*agent|job.*api|uvicorn|fastapi' | head -n1 || true)"
fi

if [ -n "$candidate" ]; then
  systemctl enable --now "$candidate" || true
  systemctl restart "$candidate" || true
  sleep 3
fi

if ! curl -fsS --max-time 5 http://127.0.0.1:8000/health >/dev/null 2>&1; then
  docker_name="$(docker ps -a --format '{{.Names}} {{.Ports}}' 2>/dev/null | awk '/8000->|:8000\//{print $1; exit}' || true)"
  if [ -n "$docker_name" ]; then
    docker restart "$docker_name" || true
    sleep 3
  fi
fi

echo "service_candidate=${candidate:-none}"
...
AGENT_SERVICE="$(detect_agent_service || true)"
echo "Detected agent service from VM2: ${AGENT_SERVICE:-none}"
echo

for host in "$VM1" "$VM2" "$VM3"; do
  repair_agent "$host" "${AGENT_SERVICE:-}"
  sleep 1
done
