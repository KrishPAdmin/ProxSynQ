...
# ---------------------------------------------------------------------------
# Background cache — keeps a recent snapshot so page loads are instant
# ---------------------------------------------------------------------------
_cache_lock = threading.Lock()
_cache: dict = {}
_cache_ts: float = 0.0
CACHE_TTL = 3.0  # seconds — avoids hammering nodes on rapid requests
...
# ---------------------------------------------------------------------------
# Batch Prometheus queries — one request per expression, parse all instances
# ---------------------------------------------------------------------------

_PROM_EXPRESSIONS = {
    "cpu": '100 - (avg by (instance) (rate(node_cpu_seconds_total{job="node_exporter",mode="idle"}[5m])) * 100)',
    "mem": '100 * (1 - (node_memory_MemAvailable_bytes{job="node_exporter"} / node_memory_MemTotal_bytes{job="node_exporter"}))',
    "root_used": '100 * (1 - (node_filesystem_avail_bytes{job="node_exporter",mountpoint="/",fstype!~"rootfs|tmpfs|overlay"} / node_filesystem_size_bytes{job="node_exporter",mountpoint="/",fstype!~"rootfs|tmpfs|overlay"}))',
    "shared_used": '100 * (1 - (node_filesystem_avail_bytes{job="node_exporter",mountpoint="/srv/proxsyncq/shared"} / node_filesystem_size_bytes{job="node_exporter",mountpoint="/srv/proxsyncq/shared"}))',
    "load1": 'node_load1{job="node_exporter"}',
    "cores": 'count by (instance) (count without (cpu,mode) (node_cpu_seconds_total{job="node_exporter",mode="idle"}))',
    "up_exporter": 'up{job="node_exporter"}',
    "up_agent": 'up{job="node_agent"}',
}


def _prom_batch_fetch() -> dict[str, list]:
    """Fetch all Prometheus expressions in parallel, return {key: results_list}."""
    out: dict[str, list] = {}

    def _fetch(key, expr):
        return key, prom_query(expr)

    with ThreadPoolExecutor(max_workers=8) as pool:
        futures = {pool.submit(_fetch, k, v): k for k, v in _PROM_EXPRESSIONS.items()}
...
def _extract_value(results: list, instance: str):
    for item in results:
        if item.get("metric", {}).get("instance") == instance:
            try:
                return round(float(item["value"][1]), 2)
...
def _extract_bool(results: list, instance: str):
    for item in results:
        if item.get("metric", {}).get("instance") == instance:
            try:
                return int(float(item["value"][1])) == 1
...
def get_metrics_from_batch(node, prom_data: dict):
    ...
    exporter_up = _extract_bool(prom_data.get("up_exporter", []), exporter_instance)
    agent_up = _extract_bool(prom_data.get("up_agent", []), agent_instance) if agent_instance else None
    ...
    cpu = _extract_value(prom_data.get("cpu", []), exporter_instance)
    mem = _extract_value(prom_data.get("mem", []), exporter_instance)
    root_used = _extract_value(prom_data.get("root_used", []), exporter_instance)
    shared_used = _extract_value(prom_data.get("shared_used", []), exporter_instance)
    load1 = _extract_value(prom_data.get("load1", []), exporter_instance)
    cores = _extract_value(prom_data.get("cores", []), exporter_instance)
    load_pct = None
...
def recent_sync_events(limit=2000):
    try:
        with db_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT node_name, event_type, file_path,
                           from_node, detail, occurred_at
                    FROM sync_events
...
def recent_conflicts(limit=1000):
    try:
        with db_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT file_path, node_a, node_b,
                           hash_a, hash_b, detected_at, resolved
                    FROM conflicts
...
def recent_arbiter_actions(limit=500):
    try:
        with db_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT action, target, reason,
                           proxmox_task, triggered_at, outcome
    ...
    for k, v in dict(row).items():
        if hasattr(v, "isoformat"):
            if hasattr(v, "tzinfo") and v.tzinfo is not None:
                out[k] = v.astimezone(TZ_TORONTO).strftime("%Y-%m-%d %H:%M:%S %Z")
            else:
...
def live_rows():
    """Build node status rows with PARALLEL health checks and batched Prom queries."""
    nodes = get_nodes()

    # 1) Fire all health checks + all prom queries in parallel
    health_results: dict[str, dict] = {}

    def _check_health(node):
        return node["name"], get_health(node)

    with ThreadPoolExecutor(max_workers=6) as pool:
        # health checks (one per node)
        health_futures = {pool.submit(_check_health, n): n["name"] for n in nodes}
        # prom batch (runs concurrently with health checks)
        prom_future = pool.submit(_prom_batch_fetch)
        ...
        for f in as_completed(health_futures):
            try:
                ...
                name = health_futures[f]
                health_results[name] = {
                    "reachable": False, "status_code": 0,
                    "data": None, "error": "check failed",
                }

        try:
            prom_data = prom_future.result()
        except Exception:
            prom_data = {}

    # 2) Assemble rows (instant — no network calls)
    rows = []
    for node in nodes:
        row = {
            "node": node,
            "health": health_results.get(node["name"], {
                "reachable": False, "status_code": 0,
                "data": None, "error": "unknown",
            }),
            "metrics": get_metrics_from_batch(node, prom_data),
        }
    ...
    for key, value in dict(job).items():
        if hasattr(value, "isoformat"):
            if hasattr(value, "tzinfo") and value.tzinfo is not None:
                out[key] = value.astimezone(TZ_TORONTO).strftime("%Y-%m-%d %H:%M:%S %Z")
            else:
...
def build_dashboard_payload():
    """Build the full dashboard payload with parallel data fetching."""
    # Run DB queries + live_rows in parallel
    with ThreadPoolExecutor(max_workers=6) as pool:
        f_rows    = pool.submit(live_rows)
        f_jobs    = pool.submit(recent_jobs)
        f_sync    = pool.submit(recent_sync_events)
        f_conf    = pool.submit(recent_conflicts)
        f_arb     = pool.submit(recent_arbiter_actions)
        f_rabbit  = pool.submit(rabbit_queue_summary)

        rows        = f_rows.result()
        raw_jobs    = f_jobs.result()
        raw_sync    = f_sync.result()
        raw_conf    = f_conf.result()
        raw_arb     = f_arb.result()
        queue       = f_rabbit.result()

    jobs         = [serialize_job(job) for job in raw_jobs]
    sync_events  = [serialize_row(r)   for r in raw_sync]
    conflicts    = [serialize_row(r)   for r in raw_conf]
    arb_actions  = [serialize_row(r)   for r in raw_arb]
...
def get_cached_payload():
    """Return cached payload if fresh, otherwise rebuild."""
...
def index(request: Request):
    ...
    # Cache this payload so the first AJAX refresh is instant too
    global _cache, _cache_ts
    with _cache_lock:
        _cache = payload
        _cache_ts = time.monotonic()
    ...
    return templates.TemplateResponse(
        "index.html",
        {
            ...
            "rows": payload["rows"],
            "jobs": payload["jobs"],
            "sync_events": payload["sync_events"],
            "conflicts": payload["conflicts"],
            "arbiter_actions": payload["arbiter_actions"],
            "failover_ring": payload["failover_ring"],
            "updated_at": payload["updated_at"],
            "message": request.query_params.get("message", ""),
            "status": request.query_params.get("status", ""),
            "ui_marker": "ui-instant-load-v1",
            # Full payload as JSON for instant JS hydration
...
def submit_job(
    ...
    # Invalidate cache so next refresh shows the new jobs
    global _cache_ts
    with _cache_lock:
        _cache_ts = 0.0
