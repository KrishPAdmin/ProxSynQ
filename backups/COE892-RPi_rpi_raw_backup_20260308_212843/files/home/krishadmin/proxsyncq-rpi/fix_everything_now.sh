...
VM3="10.26.0.173"
VMS=("${VM1}" "${VM2}" "${VM3}")
UI_MARKER="ui-final-direct-pi-v7"
...
<body>
  ...
  <script>
    (function () {
      console.log("ui-final-direct-pi-v7");
...
deploy_node_agent() {
  local host="$1"
  echo "== deploying node agent on ${host} =="
...
cat > /home/krishadmin/proxsyncq-node-agent/Dockerfile <<'DOCKER'
...
cat > /home/krishadmin/proxsyncq-node-agent/docker-compose.yml <<'YAML'
services:
  node_agent:
    build: .
    container_name: proxsyncq-node-agent
...
if [ -n "${pid}" ]; then
  unit="$(sudo systemctl status "${pid}" --no-pager 2>/dev/null | awk 'NR==1{for(i=1;i<=NF;i++) if($i ~ /\.service$/){gsub(/^●/, "", $i); print $i; exit}}' || true)"
  if [ -n "${unit}" ]; then
    sudo systemctl stop "${unit}" || true
    sleep 2
  fi
  if sudo ss -ltnp '( sport = :8000 )' 2>/dev/null | grep -q ':8000'; then
    sudo kill "${pid}" || true
    sleep 2
  fi
fi
...
sudo apt-get install -y curl ca-certificates prometheus-node-exporter >/dev/null 2>&1 || true
sudo systemctl enable --now prometheus-node-exporter || true
sudo systemctl restart prometheus-node-exporter || true

cd /home/krishadmin/proxsyncq-node-agent
...
echo "== UI API check =="
curl -fsS http://127.0.0.1:8080/api/dashboard | sed -n '1,80p'
echo
